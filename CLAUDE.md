...
SysAccounts — Linux server user management web app. Backend executes system commands (`useradd`, `userdel`, `groupadd`, `who`, `last`, etc.) via Express API. Frontend is React SPA served statically in production. Authentication via Yaotoshi Accounts (OAuth 2.0 PKCE).
...
- **Auth**: OAuth 2.0 PKCE via `@yaotoshi/auth-sdk` — backend validates tokens, frontend handles login flow
...
- **Auth middleware**: `server/middleware/auth.js` validates Bearer tokens against Yaotoshi Accounts `/me` endpoint with in-memory cache (5 min TTL)
...
- OAuth 2.0 PKCE via Yaotoshi Accounts (`@yaotoshi/auth-sdk`)
